(function(){
  ...
  async function run(){
    try{
      ...
      // Admin role check: prefer v2 then fall back to legacy to avoid ambiguous overload errors
      let admin=null; try{ let adminData=null, adminError=null; try { const { data, error } = await sb.rpc('check_admin_access_v2', { p_required_role: 'admin', p_ip_address: null }); adminData=data; adminError=error; } catch(v2Err){ const missing=/does not exist|not found/i.test(v2Err.message||''); if(!missing){ adminError=v2Err; } else { try { const { data, error } = await sb.rpc('check_admin_access', { p_required_role: 'admin', p_ip_address: null }); adminData=data; adminError=error; } catch(legacyErr){ adminError=legacyErr; } } } if(adminError){ S('scAdmin').innerHTML='<span class="bad">RPC error</span>'; S('scAdminRaw').textContent=adminError.message||String(adminError);} else { admin=adminData; const granted = admin?.access_granted || admin?.has_access; S('scAdmin').innerHTML = granted?`<span class="ok">GRANTED (${admin.role_type||'admin'})</span>`:'<span class="bad">DENIED</span>'; S('scAdminRaw').textContent=JSON.stringify(admin,null,2);} } catch(e){ S('scAdmin').innerHTML='<span class="bad">Exception</span>'; S('scAdminRaw').textContent=e.message; }
