...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setTimeout(async () => { if (typeof AnonymousAccessModal !== 'undefined'){ let authReady=false; let attempts=0; while(!authReady && attempts<30){ if(window.supabaseClient || window.supabase || window.sb){ authReady=true; break; } await new Promise(r=>setTimeout(r,200)); attempts++; } if(authReady){ window.anonymousAccessModal=new AnonymousAccessModal(); console.log('🔒 Anonymous access modal initialized after auth ready'); } else { console.log('⚠️ Auth systems not ready, skipping anonymous modal'); } } else { console.warn('⚠️ AnonymousAccessModal not loaded'); } },2000);
