...
async function saveProfileChanges(){ const formData={ display_name:document.getElementById('editDisplayName').value.trim(), username:document.getElementById('editUsername').value.replace('@','').trim(), bio:document.getElementById('editBio').value.trim() }; const locationInput=document.getElementById('sheetLocationInput'); formData.location=locationInput ? locationInput.value.trim() : ''; if(!formData.display_name){ showToast('Display name is required','error'); return; } if(!formData.username || formData.username.length<3){ showToast('Username must be at least 3 characters','error'); return; } if(formData.bio.length>160){ showToast('Bio must be 160 characters or less','error'); return; } try{ Object.assign(currentProfile, formData); await saveProfileToStorage(currentProfile); updateProfileDisplay(currentProfile); closeEditProfile(); showToast('Profile saved successfully! 🎉','success'); } catch(error){ console.error('❌ Profile save error:', error); showToast('Failed to save profile. Please try again.','error'); } }
async function saveProfileToStorage(profile, userId=null){ const profileData={ ...profile, last_updated:new Date().toISOString(), version:'1.0' }; try{ const storageKey=userId ? `stayhi_profile_${userId}` : 'stayhi_profile_demo'; localStorage.setItem(storageKey, JSON.stringify(profileData)); if(window.supabaseClient && userId){ profileData.user_id=userId; await saveProfileToSupabase(profileData); } else if(!userId){ console.log('🎭 Demo profile - skipping Supabase sync'); } sessionStorage.setItem(`stayhi_profile_backup_${userId || 'demo'}`, JSON.stringify(profileData)); } catch(error){ console.error('Storage save error:', error); throw error; } }
async function saveProfileToSupabase(profile){ try{ const { data, error }=await window.supabaseClient.from('profiles').upsert({ id:profile.id || profile.username, username:profile.username, display_name:profile.display_name, bio:profile.bio, location:profile.location, avatar_url:profile.avatar_url, updated_at:new Date().toISOString() }, { onConflict:'username' }); if(error){ console.warn('⚠️ Supabase save failed:', error.message); } else { console.log('☁️ Profile synced to Supabase'); } } catch(error){ console.warn('⚠️ Supabase unavailable:', error.message); } }
