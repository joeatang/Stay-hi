...
// 🏆 WOZ-GRADE Profile System - PROFILEMANAGER INTEGRATED
// NO hardcoded defaults - database-first loading
let currentProfile = {
  username: '',
  display_name: '',
  bio: '', // NO DEFAULT - loads from database
  location: '',
  avatar_url: null,
    ...
    try {
      socialAvatarUploader = new window.SocialAvatarUploader({
        onSave: async (blob, originalFile) => {
          // 🎯 TIER CHECK: Validate avatar upload permission
          ...
          if (!features.avatarUpload) {
            console.log('🚫 Avatar upload blocked for tier:', tier);
...
function updateProfileDisplay(profileData){ 
  ...
  console.warn('❌ BLOCKED: profile-main.js updateProfileDisplay() is deprecated');
  console.warn('➡️ Use profile.html inline updateProfileDisplay() instead');
  return; // Exit immediately - don't overwrite display
  
  // OLD CODE BELOW - NEVER RUNS
  const usernameEl=document.getElementById('profileUsername'); if(usernameEl && profileData.username) usernameEl.textContent=`@${profileData.username}`; const displayNameEl=document.getElementById('profileDisplayName'); if(displayNameEl) displayNameEl.textContent=profileData.display_name || 'Stay Hi User'; const bioEl=document.getElementById('profileBio'); if(bioEl) bioEl.textContent=profileData.bio || 'No bio yet. Click edit to add one!'; const locationEl=document.getElementById('profileLocation'); if(locationEl) locationEl.textContent=profileData.location || 'Location not set'; const joinedEl=document.getElementById('profileJoined'); if(joinedEl && profileData.created_at){ const joinDate=new Date(profileData.created_at); const monthNames=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']; joinedEl.textContent=`Joined ${monthNames[joinDate.getMonth()]} ${joinDate.getFullYear()}`; } if(profileData.avatar_url){ updateProfileAvatar(profileData.avatar_url); } const profileInfo=document.querySelector('.profile-info'); if(profileInfo){ profileInfo.setAttribute('data-profile-loading','false'); } }
function updateStatsDisplayLegacy(statsData){ 
  console.warn('⚠️ DEPRECATED: updateStatsDisplayLegacy() called - use profile.html updateStatsDisplay() instead');
  // This function is deprecated - stats should be loaded from user_stats table directly
  // Keeping for backward compatibility but don't use it
...
function navigateToHiDashboard(){ window.location.href=(window.hiPaths?.page?window.hiPaths.page('dashboard'):'hi-dashboard.html'); }
