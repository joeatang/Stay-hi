...
-- Users can read their own balance and ledger
create policy if not exists "hi_points_select_self" on public.hi_points for select to authenticated using ( auth.uid() = user_id );
create policy if not exists "hi_points_ledger_select_self" on public.hi_points_ledger for select to authenticated using ( auth.uid() = user_id );

-- Service role can modify
create policy if not exists "hi_points_all_service" on public.hi_points for all to service_role using ( true ) with check ( true );
create policy if not exists "hi_points_ledger_all_service" on public.hi_points_ledger for all to service_role using ( true ) with check ( true );

create index if not exists hi_points_ledger_user_ts_idx on public.hi_points_ledger(user_id, ts desc);

-- Helper function to award points (service-only)
