-- 🔧 FIX STREAK CALCULATION: Include check-ins + shares
-- Streaks should count BOTH medallion taps (check-ins) AND shares
...
BEGIN
  ...
  FROM (
    ...
    UNION
    SELECT checkin_date as activity_date FROM hi_points_daily_checkins WHERE user_id = user_uuid
  ) all_activity;
...
-- Recalculate with correct streak logic
DO $$
DECLARE
  user_record RECORD;
...
BEGIN
  ...
  RAISE NOTICE '✅ Recalculated stats with check-ins for % users', processed_count;
END $$;

-- Verify your streak is now correct
SELECT 
  us.user_id,
  us.total_hi_moments,
  us.current_streak,
  ...
  (SELECT COUNT(*) FROM hi_points_daily_checkins WHERE user_id = us.user_id) as total_checkins
FROM user_stats us
