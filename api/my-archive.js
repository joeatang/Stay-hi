// api/my-archive.js
// Plain Node.js style for Vercel functions (no Next/Edge APIs)
...
function readSession(req) {
  try {
    ...
    if (!raw) return null;
    // try JSON first, then base64url → JSON
...
export default async function handler(req, res) {
  ...
  if (body?.action === 'health') return send(res, 200, { ok: true, route: 'my-archive' });
