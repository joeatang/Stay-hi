-- ========================================
-- 🔧 FIX: Capture membership ID correctly
-- ========================================
...
BEGIN
  ...
  -- Create membership record if tier is granted and capture the membership ID
  IF v_grants_tier IS NOT NULL THEN
    INSERT INTO user_memberships (user_id, tier, status, invitation_code)
    VALUES (p_user_id, v_grants_tier, 'active', p_code)
    ON CONFLICT (user_id) DO UPDATE
    SET tier = EXCLUDED.tier, status = EXCLUDED.status, invitation_code = EXCLUDED.invitation_code
    RETURNING id INTO v_membership_id;
  END IF;

  -- Log the transaction with the correct membership_id
...
SELECT 'use_invite_code function fixed - captures membership.id correctly!' AS status;
