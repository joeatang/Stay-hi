...
async function getUnifiedFeed(userId, options = {}) {
  try {
    ...
    if (type === 'all' || type === 'shares') {
      // Determine if this is for public feed (Hi Island) or user feed (Dashboard)
      const isPublicFeed = options.publicFeed === true;
      
      let sharesResult;
      if (isPublicFeed) {
        // Hi Island: Get public/anonymous shares only
        sharesResult = await HiBase.shares.getPublicShares({
          visibility: ['public', 'anonymous'], // Exclude private
          limit: type === 'shares' ? limit : Math.ceil(limit * 0.7),
          cursor: options.cursor // Support pagination
        });
        
        // Track telemetry for public feed
        const { trackEvent } = await import('../monitoring/HiMonitor.js');
        trackEvent('feed_load', {
          path: 'hibase',
          count: sharesResult.data?.length || 0,
          type: 'public_feed'
        });
      } else {
        // Dashboard: Get user's private shares
        sharesResult = await HiBase.shares.getUserShares(userId, {
          limit: type === 'shares' ? limit : Math.ceil(limit * 0.7),
      ...
      if (sharesResult.data) {
        sharesResult.data.forEach(share => {
          feedItems.push({
            id: `share_${share.id}`,
            type: share.type || 'share', // Include type for Hi5 badge display
            ...
            isPublic: share.visibility === 'public' || share.visibility === 'anonymous',
            originalData: share
          });
        });
      }
