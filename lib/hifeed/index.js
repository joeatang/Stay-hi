 ...
 * 
 * Usage:
 *   const { getUnifiedFeed } = require('./lib/hifeed');
 *   const feed = await getUnifiedFeed(userId, { page: 1, limit: 20 });
 */

// Browser-compatible imports
let HiBase, HiMonitor;

// Dynamic imports for browser compatibility
if (typeof window !== 'undefined') {
  // Browser environment - use global objects
  HiBase = window.HiBase;
  HiMonitor = window.HiMonitor || { logError: console.error, trackEvent: console.log };
} else {
  // Node environment - use require
  try {
    HiBase = require('../hibase');
    const monitoring = require('../monitoring/HiMonitor');
    HiMonitor = { logError: monitoring.logError, trackEvent: monitoring.trackEvent };
  } catch (e) {
    console.warn('HiFeed: Running without HiBase/HiMonitor dependencies');
    HiBase = null;
    HiMonitor = { logError: console.error, trackEvent: console.log };
  }
}
