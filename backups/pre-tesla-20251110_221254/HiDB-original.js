...
(function() {
  ...
  async function insertPublicShare(entry) {
    ...
    const row = {
      ...
      content: shareContent, // Actual field: content (not text)
      visibility: entry.isAnonymous ? 'anonymous' : 'public', // Actual field: visibility
      share_type: entry.type === 'higym' ? 'reflection' : 'hi5', // Tesla: Enhanced type mapping
      location_data: entry.location ? { location: entry.location } : null, // Actual field: location_data
      metadata: {
        currentEmoji: entry.currentEmoji,
        currentName: entry.currentName,
        desiredEmoji: entry.desiredEmoji, 
        desiredName: entry.desiredName,
        origin: entry.origin || detectOrigin(),
  ...
  async function insertArchive(entry) {
    ...
    const row = {
      user_id,
      content: shareContent, // Actual field: content (not journal/text)
      share_type: entry.type === 'higym' ? 'reflection' : 'hi5', // Map types to schema enum
      visibility: entry.visibility || 'private', // Always private for archives unless specified
      location_data: entry.location ? { location: entry.location } : null, // Actual field: location_data
      metadata: {
        currentEmoji: entry.currentEmoji,
        desiredEmoji: entry.desiredEmoji,
        origin: entry.origin || 'hi5',
        type: entry.type || 'self_hi5',
        teslaEnhanced: true // Tesla marker
      }
    };
