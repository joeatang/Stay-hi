-- ========================================
-- 🔧 FINAL FIX: use_invite_code with auto-create users entry
-- ========================================
...
BEGIN
  ...
  -- CRITICAL FIX: Ensure users table entry exists first
  ...
  IF v_grants_tier IS NOT NULL THEN
    INSERT INTO user_memberships (user_id, tier, status, invitation_code, created_at, updated_at)
    VALUES (p_user_id, v_grants_tier, 'active', p_code, NOW(), NOW())
    ON CONFLICT (user_id) DO UPDATE
    SET tier = EXCLUDED.tier, status = EXCLUDED.status, invitation_code = EXCLUDED.invitation_code, updated_at = NOW()
...
SELECT 'use_invite_code function fixed - auto-creates users entry!' AS status;
