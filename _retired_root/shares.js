 ...
 * @param {Array<string>} options.visibility - Visibility filters ['public', 'anonymous'] 
 * @param {number} options.limit - Number of shares to fetch
 * @param {string} options.cursor - Pagination cursor
 * @returns {Object} { data, error } from Supabase
 */
async function _getPublicShares(options = {}) {
    const { visibility = ['public', 'anonymous'], limit = 50, cursor } = options;
    
    return hiBaseClient.execute(async (client) => {
        let query = client
            .from('public_hi_feed')
            .select('*')
            .in('visibility', visibility)  // Only public/anonymous, never private
 ...
 * @param {Object} shareData - Hi share data (user_id will be auto-detected from auth)
...
export async function createHiShare(shareData) {
    ...
    try {
        ...
        if (authData.isAnon || !authData.userId) {
            return {
                data: null,
                error: { message: 'Authentication required to create Hi share', code: 'AUTH_REQUIRED' }
            };
        }
 ...
 * @param {Object} options - Fetch options (userId auto-detected from auth)
 * @returns {Object} { data, error } with user's shares and stats
 */
export async function getUserHiHistory(options = {}) {
    const {
        limit = 20,
        offset = 0,
    ...
    try {
        ...
        if (authData.isAnon || !authData.userId) {
            return {
                data: null,
                error: { message: 'Authentication required to view Hi history', code: 'AUTH_REQUIRED' }
            };
        }
        userId = authData.userId;
    } catch (error) {
