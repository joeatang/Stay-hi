 ...
 * 
 * Tesla-Grade Dual Tracking:
 * - Increments global Hi Waves counter (1:1 ratio)
 * - If user is authenticated, also increments their personal wave count
 * 
 * @param {string|null} userId - User ID or null for anonymous
 * @returns {Promise<{data: {globalWaves: number, userWaves: number|null}, error: string|null}>}
 */
...
/**
 * METRICS SEPARATION: Get user-specific Hi Waves count 
 * @param {string} userId - User ID to get waves for
 * @returns {Promise<{data: number|null, error: string|null}>}
 */
const getUserHiWaves = withTelemetry('getUserHiWaves', async (userId) => {
    if (!userId) {
        return {
            data: null,
    ...
    return hiBaseClient.execute(async (client) => {
        const { data, error } = await client.rpc('get_user_hi_waves', {
            user_id: userId
        });
        ...
        if (error) {
            return {
                data: null,
                error: `Failed to get user Hi Waves: ${error.message}`
            };
        }
        
        const userWaveCount = data?.data || 0;
...
const getMetrics = withTelemetry('getMetrics', async () => {
    ...
    // TEMP FIX: Check if functions failed (database deployment pending)
    if (wavesResult.error && hi5sResult.error) {
        console.warn('⚠️ Database functions not deployed, using temporary separation');
        
        // Import and use temporary separation
        const { getTempSeparatedMetrics } = await import('./temp-separation.js');
        return await getTempSeparatedMetrics();
    }
...
export { getHiWaves, getTotalHi5s, getUserHiWaves, insertMedallionTap, getMetrics };
